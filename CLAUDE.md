...
5. **Ablauf in kindle_capture.py (Hotkey-basiert, neues WinUI-Kindle):**
   ...
   6. **Seitenformat von der Titelseite bestimmen** (`detect_page_region_from_cover`): Die letterboxte Titelseite hebt sich mit komplett schwarzem/weißem Rand vom Hintergrund ab. Schlägt die Erkennung fehl → **fail-loud** mit Beweis-Screenshot `_debug_titelseite.png` im Buch-Ordner. Erkannt wird ihr Rechteck über **Abdeckung + Rand-Test**, NICHT über Varianz:
      - **Spalten:** Eine Spalte durch die Seite ist über den Großteil ihrer Höhe Inhalt, die Titelleiste bedeckt nur ~6% → Abdeckungs-Schwelle (50%) wirft die Chrome raus. Längster zusammenhängender Block = Seite.
      - **Zeilen:** Die Titelleiste ist über die **volle Breite** Inhalt, Abdeckung allein würde sie für Seite halten. Aber eine echte Seitenzeile hat **Hintergrund im Seitenrand daneben** — dieser Rand-Test trennt Seite von Chrome.
      - ⚠️ **Warum nicht Varianz:** Chrome hat auch Varianz — der Varianz-Scan lieferte gemessen `1443x834` (Querformat, ganzes Fenster) statt `516x804` (Cover).
